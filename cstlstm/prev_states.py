...
class PreviousStates:
    ...
    def __call__(self, level_nodes, level_up_wirings, prev_outputs):
        ...
        cell_states = self.states(level_nodes, level_length, prev_outputs[0], level_up_wirings)

        # grab the hidden states
        hidden_states = self.states(level_nodes, level_length, prev_outputs[1], level_up_wirings)
    ...
    def children(prev_out, child_ixs_level_i):
        ...
        selector = torch.LongTensor(child_ixs_level_i).to(prev_out.device)
